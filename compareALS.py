...
from metricsALS import comparison, rhInfo
...
class plot(object):
  def __init__(self, region, dates, gtxs, ρg, ρv):
    self.region = region
    ...
    for date, gtx in zip(dates, gtxs):
      comp = comparison(region, date, gtx, ρg, ρv)
...
if __name__=='__main__':
  regionList = np.array(['deju', 'ornl', 'sodankyla', 'sonoma', 'wref'])
  ρvρgArr = np.arange(0.5, 1.51, 0.1)
  cvBias = np.empty((len(regionList), len(ρvρgArr)))
  cvNgBias = np.empty((len(regionList), len(ρvρgArr)))
  ρvρgFit = np.empty(len(regionList))
  for i in range(len(regionList)):
    ...
    ρvρgFit[i] = rates.ρvρg.mean()
    dateSim, gtxSim = np.array([[d, g] for d, g in zip(dates, gtxs)\
                                if os.path.exists(os.path.join(p.simDir,
                                                  '.'.join([d, g, 'sim.pkl'])))]).T
    for j in range(len(ρvρgArr)):
      comp = plot(region, dateSim, gtxSim, 1./ρvρgArr[j], 1)
      Ng50 = comp.real.where(comp.real.Ng>=50)
      cvBias[i,j] = np.mean(comp.real.cv - comp.real.cv_ALS)
      cvNgBias[i,j] = np.nanmean(Ng50.cv - Ng50.cv_ALS) 
   
  ds = xr.Dataset()
  ds.coords['rhovrhog'] = ('rhovrhog', ρvρgArr)
  ds.coords['region'] = ('region', regionList)
  ds['rhovrhog_est'] = ('region', ρvρgFit)
  ds['cv_bias'] = (('region', 'rhovrhog'), cvBias)
  ds['cv_Ng50_bias'] = (('region', 'rhovrhog'), cvNgBias)
  ds.to_netcdf('cv_als_bias.nc') 

  fig, ax = plt.subplots(1,1)
  ax.axhline(0, lw=1, c='k')
  ax.axvline(1, lw=1, c='k')
  for i in range(len(regionList)):
    ax.plot(ds.rhovrhog, ds.cv_bias[i,:], label=getRegionName(regionList[i]), color=cmap((i+1)/(2+len(regionList))))
    ax.axvline(ds.rhovrhog_est[i], color=cmap((i+1)/(2+len(regionList))), ls=':')
  ax.set_xlim(0.5, 1.5)
  ax.set_ylim(-15, 15)
  ax.set_xticks(np.arange(0.5, 1.51, 0.25))
  ax.set_yticks(np.arange(-15, 16,5))
  ax.set_xlabel('ρv/ρg')
  ax.set_ylabel('Mean canopy cover bias (%, ICESat-2 - ALS)')
  ax.legend(loc='best', fancybox=False, edgecolor='none')
  fig.tight_layout()
  fig.savefig('canopycoverbias.ρvρg.inverted.withlines.pdf')
